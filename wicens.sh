#!/bin/sh
# shellcheck disable=SC2039,SC2183,SC2104,SC1090,SC2154,SC2034,SC3003,SC3043,SC3045,SC3046,SC3057,SC3061
# hex expr   menuprintf   continuefunc   constantsource   unassvar   unusedvar   local   strindex
# SC3003,SC3043,SC3045,SC3046,SC3057,SC3061: BusyBox ash extensions that work but ShellCheck flags as POSIX-incompatible
################################################################################
#                                 _                                            #
...
F_random_num() {
  ...
  # Use od with awk - simpler and works on BusyBox
  od -An -N2 -tu2 /dev/urandom | awk -v max="${random_max}" '{print ($1 % max) + 1}'
}

F_random_num_end() { : ; } # random_num
...
F_ministun() {
	...
	while [ "${stun_count}" -gt 0 ] ; do
		...
		while [ "${server_attempt}" -lt "${stun_list}" ] ; do
			...
			if current_wan_ip="$(ministun -t 1000 -c 1 "${stun_server}" 2>/dev/null)" ; then
				break
			fi
